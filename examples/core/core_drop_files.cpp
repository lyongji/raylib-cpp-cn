...
int main() {
    ...
    R窗口 窗口(屏幕宽, 屏幕高, "raylib [core] example - drop files", FLAG_WINDOW_RESIZABLE | FLAG_WINDOW_HIGHDPI);
    ...
    while (!窗口.是已关闭()) { // Detect window close button or ESC key
        ...
        窗口.开始绘制();
        {
            ...
            if (拖放文件.empty()) {
                raylib::绘制文本("Drop your files to this window!", 100, 40, 20, 文本颜色.透明度(0.5));
            }
            else {
                ...
                for (int i = 0; i < (int)拖放文件.size(); i++) {
                    ...
                    raylib::绘制文本(拖放文件[i].c_str(), 120, 100 + 40 * i, 10, R颜色::灰());
                }
                raylib::绘制文本("Drop new files...", 100, 110 + 40 * (int)拖放文件.size(), 20, R颜色::深灰());
            }
        }
        窗口.结束绘制();
        //----------------------------------------------------------------------------------
