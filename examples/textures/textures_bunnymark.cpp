...
public:
    兔子() {
        ...
        color = R颜色(GetRandomValue(50, 240), GetRandomValue(80, 240), GetRandomValue(100, 240));
    }
    ...
    void 更新(const raylib::纹理2D& texBunny) {
        ...
        if (((position.x + texBunny.width / 2) > GetScreenWidth()) || ((position.x + texBunny.width / 2) < 0))
            speed.x *= -1;
        if (((position.y + texBunny.height / 2) > GetScreenHeight()) || ((position.y + texBunny.height / 2 - 40) < 0))
            speed.y *= -1;
    }
...
int main() {
    ...
    while (!窗口.是已关闭()) { // Detect window close button or ESC key
        ...
        while (窗口.绘制中()) {
            ...
            raylib::绘制文本(TextFormat("bunnies: %i", bunnies.size()), 120, 10, 20, GREEN);
            raylib::绘制文本(
