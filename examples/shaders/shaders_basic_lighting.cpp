 ...
 *software
 *
 *   Copyright (c) 2019-2024 Chris Camacho (@codifies) and Ramon Santamaria
 *(@raysan5)
 *
 ********************************************************************************************/
...
int main() {
  ...
  着色器.locs[SHADER_LOC_VECTOR_VIEW] = 着色器.取统一变量位置("viewPos");
  ...
  int ambientLoc = 着色器.取统一变量位置("ambient");
  std::array<float, 4> ambientValues = {0.1f, 0.1f, 0.1f, 1.0f};
  着色器.设统一变量值(ambientLoc, ambientValues.data(), SHADER_UNIFORM_VEC4);
  ...
  while (!WindowShouldClose()) // Detect window close button or ESC key
  {
    ...
    // 0.0f, 0.0f })
    std::array<float, 3> 相机位置 = {相机.position.x, 相机.position.y,
                                     相机.position.z};
    SetShaderValue(着色器, 着色器.locs[SHADER_LOC_VECTOR_VIEW], 相机位置.data(),
                   SHADER_UNIFORM_VEC3);
    ...
    for (const auto &灯光 : 灯光组) {
      if (灯光.enabled)
        ...
        DrawSphereWires(灯光.position, 0.2f, 8, 8,
                        ColorAlpha(灯光.color, 0.3f));
    }
