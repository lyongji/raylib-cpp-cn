 ...
 *
 *   NOTE: raylib is distributed with some free to use fonts (even for
 *commercial pourposes!) To view details and credits for those fonts, check
 *raylib license file
 *
...
int main() {
  ...
  for (int i = 0; i < 字体组.size(); i++) {
    auto size =
        字体组[i].测量文本(messages[i], 字体组[i].baseSize * 2, spacings[i]);
  ...
  std::array<raylib::颜色, MAX_FONTS> 颜色组 = {
  ...
  while (!窗口.检查是否已关闭()) { // Detect window close button or ESC key
    ...
    while (窗口.绘制中()) {
      ...
      for (int i = 0; i < 字体组.size(); i++) {
        字体组[i].绘制文本(messages[i], positions[i], 字体组[i].baseSize * 2,
                           spacings[i], 颜色组[i]);
      }
    }
