 ...
 *
 *   After font loading, font texture atlas filter could be configured for a
 *softer display of the font when scaling it to different sizes, that way, it's
 *not required to generate multiple fonts at multiple sizes (as long as the
 *scaling is not very different)
 *
...
int main() {
  ...
  while (!窗口.是已关闭()) // Detect window close button or ESC key
  {
    ...
    while (窗口.绘制中()) {
      ...
      // offsets... DrawRectangleLines(fontPosition.x, fontPosition.y,
