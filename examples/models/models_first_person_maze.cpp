...
int main() {
  ...
  R非托管网格 网格 = R非托管网格::生成立方体贴图网格(
      立方图,
      Vector3{1.0f, 1.0f,
              1.0f}); // Use MeshUnmanaged, Mesh will be handled by Model
  R模型 模型(网格);

  // NOTE: By default each cube is mapped to one part of texture atlas
  R纹理 纹理("resources/cubicmap_atlas.png"); // Load map texture
  模型.materials[0].maps[MATERIAL_MAP_DIFFUSE].texture =
      纹理; // Set map diffuse texture

  // Get map image data to be used for collision detection
  Color *像素图 = 立方图.加载颜色数组();
  ...
  while (!窗口.检查是否已关闭()) { // Detect window close button or ESC key
    ...
    float 玩家半径 = 0.1f; // Collision radius (player is modelled as a cilinder
                           // for collision)
    ...
    while (窗口.绘制中()) {
      ...
      相机.开始3D模式();
      {
        模型.绘制模型(映射位置); // Draw maze map
        // playerPosition.DrawCube((Vector3){ 0.2f, 0.4f, 0.2f }, RED);  // Draw
        // player
      }
      相机.结束3D模式();

      cubicmap.绘制(Vector2{static_cast<float>(GetScreenWidth() -
                                               cubicmap.width * 4 - 20),
                            20},
                    0.0f, 4.0f, WHITE);
      ...
      DrawRectangle(GetScreenWidth() - cubicmap.width * 4 - 20 + 玩家单元X * 4,
                    20 + 玩家单元Y * 4, 4, 4, RED);
