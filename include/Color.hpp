...
public:
   ...
   * 从 HSV 值返回一个颜色
   */
  static ::Color 从HSV生成(float 色调, float 饱和度, float 明度) {
    return ::ColorFromHSV(色调, 饱和度, 明度);
  }
   ...
   */
  [[nodiscard]] 颜色 透明化(float 透明度) const {
    return ::Fade(*this, 透明度);
  }
   ...
   * 绘制一条线
   */
  void 绘制线段(int 起点X, int 起点Y, int 终点X, int 终点Y) const {
    ::DrawLine(起点X, 起点Y, 终点X, 终点Y, *this);
  }

  /**
   * 使用向量点绘制一条线
   */
  void 绘制线段(::Vector2 起点, ::Vector2 终点) const {
    ::DrawLineV(起点, 终点, *this);
  }

  /**
   * 使用向量点绘制一条线，并指定线宽
   */
  void 绘制线段(::Vector2 起点, ::Vector2 终点, float 粗细) const {
  ...
  void 绘制连续线段(::Vector2 *顶点表, int 点数) const {
    ::DrawLineStrip(顶点表, 点数, *this);
  }
  ...
  void 绘制文本(const ::Font &字体, const char *文本, ::Vector2 位置,
                ::Vector2 原点, float 旋转, float 字体大小,
                float 字间距) const {
  ...
  void 绘制文本(const ::Font &字体, const std::string &文本, ::Vector2 位置,
                ::Vector2 原点, float 旋转, float 字体大小,
                float 字间距) const {
    ::DrawTextPro(字体, 文本.c_str(), 位置, 原点, 旋转, 字体大小, 字间距,
                  *this);
  }

  void 绘制矩形(int 位置X, int 位置Y, int 宽, int 高) const {
    ::DrawRectangle(位置X, 位置Y, 宽, 高, *this);
  }

  void 绘制矩形(::Vector2 位置, ::Vector2 尺寸) const {
    ::DrawRectangleV(位置, 尺寸, *this);
  }

  void 绘制矩形(::Rectangle 矩形) const { ::DrawRectangleRec(矩形, *this); }

  void 绘制矩形(::Rectangle 矩形, ::Vector2 原点, float 旋转) const {
    ::DrawRectanglePro(矩形, 原点, 旋转, *this);
  }

  void 绘制矩形边框(int 位置X, int 位置Y, int 宽, int 高) const {
    ::DrawRectangleLines(位置X, 位置Y, 宽, 高, *this);
  }

  void 绘制矩形边框(::Rectangle 矩形, float 线宽) const {
    ::DrawRectangleLinesEx(矩形, 线宽, *this);
  }
   ...
   * 返回应用了 alpha 的颜色，alpha 的范围是 0.0f 到 1.0f
   */
  [[nodiscard]] 颜色 透明度(float 透明值) const {
    return ::ColorAlpha(*this, 透明值);
  }

  /// 返回颜色插值，参数为颜色color2和插值因子factor
  颜色 插值(::Color 另一颜色, float 系数) {
    return ::ColorLerp(*this, 另一颜色, 系数);
  }

  /**
   * 返回 src 颜色与 dst 颜色进行 alpha 混合后的结果，并应用色调颜色
   */
  [[nodiscard]] 颜色 透明混合(::Color 目标, ::Color 色调) const {
    return ::ColorAlphaBlend(目标, *this, 色调);
  }
  /// raylib默认色
