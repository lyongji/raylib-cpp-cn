...
public:
   ...
   * 如果从给定的立方体贴图创建纹理失败，则抛出异常。
   *
   * @see LoadTextureCubemap()
   */
  非托管纹理(const ::Image &图像, int 布局类型) { 加载纹理(图像, 布局类型); }
   ...
   */
  非托管纹理(const std::string &文件路径) { 加载纹理(文件路径); }

  非托管纹理(::Texture &&other)
      : ::Texture{other.id, other.width, other.height, other.mipmaps,
                  other.format} {}

  GETTER(unsigned int, 纹理ID, id)
  GETTER(int, 宽, width)
  GETTER(int, 高, height)
  GETTER(int, 多级纹理数, mipmaps)
  GETTER(int, 纹理格式, format)
   ...
   * 从图像加载立方体贴图，支持多种图像立方体贴图布局。
   */
  void 加载纹理(const ::Image &图像, int 布局类型) {
   ...
   * 从文件加载纹理到 GPU 内存（VRAM）。
   */
  void 加载纹理(const std::string &文件名) {
   ...
   * 使用新数据更新 GPU 纹理矩形。
   */
  非托管纹理 &更新纹理(::Rectangle 矩形, const void *像素) {
   ...
   * @see ::DrawTextureV()
   */
  void 绘制(::Vector2 位置, ::Color 色调 = {255, 255, 255, 255}) const {
    ::DrawTextureV(*this, 位置, 色调);
  }
   ...
   * @see ::DrawTextureEx()
   */
  void 绘制(::Vector2 位置, float 旋转, float 缩放 = 1.0f,
            ::Color 色调 = {255, 255, 255, 255}) const {
   ...
   * @see ::DrawTextureRec()
   */
  void 绘制(::Rectangle 源矩形, ::Vector2 位置 = {0, 0},
            ::Color 色调 = {255, 255, 255, 255}) const {
   ...
   * @see ::DrawTexturePro()
   */
  void 绘制(::Rectangle 源矩形, ::Rectangle 目标区域, ::Vector2 原点 = {0, 0},
            float 旋转 = 0, ::Color 色调 = {255, 255, 255, 255}) const {
   ...
   */
  void 绘制(::NPatchInfo 九宫格布局, ::Rectangle 目标区域,
            ::Vector2 原点 = {0, 0}, float 旋转 = 0,
            ::Color 色调 = {255, 255, 255, 255}) const {
   ...
   * @see ::DrawBillboard()
   */
  void 绘制公告板(const ::Camera &相机, ::Vector3 位置, float 缩放,
                  ::Color 色调 = {255, 255, 255, 255}) const {
   ...
   */
  void 绘制公告板(const ::Camera &相机, ::Rectangle 源区域, ::Vector3 位置,
                  ::Vector2 尺寸, ::Color 色调 = {255, 255, 255, 255}) const {
   ...
   */
  void 绘制公告板(const ::Camera &相机, ::Rectangle 源区域, Vector3 位置,
                  ::Vector3 上方向, Vector2 尺寸, Vector2 原点,
                  float 旋转 = 0.0f,
                  ::Color 色调 = {255, 255, 255, 255}) const {
   ...
   */
  非托管纹理 &设材质纹理(::Material *材质, int 映射类型 = MATERIAL_MAP_NORMAL) {
    ::SetMaterialTexture(材质, 映射类型, *this);
    return *this;
  }

  非托管纹理 &设材质纹理(const ::Material &材质,
                         int 映射类型 = MATERIAL_MAP_NORMAL) {
   ...
   * 设置用于形状绘制的纹理和矩形。
   */
  非托管纹理 &设形状纹理(const ::Rectangle &源区域) {
   ...
   */
  非托管纹理 &设着色器纹理值(const ::Shader &着色器, int 位置索引) {
   ...
   */
  [[nodiscard]] bool 检查可用性() const { return IsTextureValid(*this); }
