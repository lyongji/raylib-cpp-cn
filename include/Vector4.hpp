...
public:
  ...
  Vector4(float x, float y) : ::Vector4{.x = x, .y = y, .z = 0, .w = 0} {}
  Vector4(float x) : ::Vector4{.x = x, .y = 0, .z = 0, .w = 0} {}
  Vector4() : ::Vector4{.x = 0, .y = 0, .z = 0, .w = 0} {}
  Vector4(::Rectangle 矩形)
      : ::Vector4{.x = 矩形.x, .y = 矩形.y, .z = 矩形.width, .w = 矩形.height} {
  }

  Vector4(::Color 颜色) { 设(ColorNormalize(颜色)); }
  ...
  [[nodiscard]] std::string 转文本() const {
    return std::format("Vector4(%f, %f, %f, %f)", x, y, z, w);
  }

  operator std::string() const { return 转文本(); }

#ifndef RAYLIB_CPP_NO_MATH
  [[nodiscard]] Vector4 乘(const ::Vector4 &vector4) const {
  ...
  [[nodiscard]] Vector4 归一化线性插值(const ::Vector4 &vector4,
                                       float 数量) const {
    return QuaternionNlerp(*this, vector4, 数量);
  }

  [[nodiscard]] Vector4 球面线性插值(const ::Vector4 &vector4,
                                     float 数量) const {
  ...
  /// 将四元数转换为轴角
  void 转轴角(::Vector3 *输出轴, float *输出角度) const {
  ...
  [[nodiscard]] Vector4 变换(const ::Matrix &矩阵) const {
    return ::QuaternionTransform(*this, 矩阵);
  }
  ...
  /// 将一个向量转换为另一个向量
  static Vector4 从Vector3转换为Vector3(const ::Vector3 &从,
                                        const ::Vector3 &到) {
    return ::QuaternionFromVector3ToVector3(从, 到);
  }

  /// 将矩阵转换为四元数
  static Vector4 从矩阵转(const ::Matrix &矩阵) {
    return ::QuaternionFromMatrix(矩阵);
  }

  /// 将一个轴和一个角度转换为四元数
  static Vector4 从轴角转(const ::Vector3 &轴, const float 角度) {
    return ::QuaternionFromAxisAngle(轴, 角度);
  }

  /// 将欧拉角转换为四元数
  static Vector4 从欧拉角转(const float 俯仰, const float yaw,
                            const float roll) {
  ...
  [[nodiscard]] Color 从归一化转为颜色() const {
