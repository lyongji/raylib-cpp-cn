...
public:
  音频流(const ::AudioStream &音频流数据) : ::AudioStream(音频流数据) {}

  音频流(rAudioBuffer *音频缓冲 = nullptr, rAudioProcessor *处理器 = nullptr,
         unsigned int 采样频率 = 0, unsigned int 采样大小 = 0,
         unsigned int 声道个数 = 0)
   ...
   */
  音频流(unsigned int 采样频率, unsigned int 采样大小,
         unsigned int 声道数 = 2) {
    加载(采样频率, 采样大小, 声道数);
  }
   ...
   * 设置新音频流的默认缓冲区大小
   */
  static void 设缓存大小为默认(int 缓冲区字节数) {
    ::SetAudioStreamBufferSizeDefault(缓冲区字节数);
  }
   ...
   * 将音频流处理器附加到流
   */
  void 绑定处理器(::AudioCallback 处理器) {
    ::AttachAudioStreamProcessor(*this, 处理器);
  }

  /**
   * 从流中分离音频流处理器
   */
  void 解绑处理器(::AudioCallback 处理器) {
    ::DetachAudioStreamProcessor(*this, 处理器);
  }
   ...
   * @throws raylib::RaylibException 如果音频流加载失败，则抛出异常。
   */
  void 加载(unsigned int 采样频率, unsigned int 采样大小,
            unsigned int 声道数 = 2) {
    卸载();
