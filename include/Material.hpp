...
public:
   ...
   * 从模型文件加载材质
   */
  static std::vector<材质> 加载材质列表(const std::string &文件路径) {
    int 材质个数 = 0;
    // TODO(RobLoach): Material::Load() 可能会导致材质数组泄漏。
    ::Material *材质数组 = ::LoadMaterials(文件路径.c_str(), &材质个数);
    return std::vector<材质>(材质数组, 材质数组 + 材质个数);
  }

  GETTERSETTER(::Shader, 着色器, shader)
  GETTERSETTER(::MaterialMap *, 材质贴图集, maps)
   ...
   * 使用材质和变换矩阵绘制3D网格
   */
  void 绘制网格(const ::Mesh &网格, ::Matrix 变换矩阵) const {
    ::DrawMesh(网格, *this, 变换矩阵);
  }

  /**
   * 使用材质和不同的变换矩阵绘制多个网格实例
   */
  void 绘制网格(const ::Mesh &网格, ::Matrix *变换矩阵数组,
                int 实例个数) const {
    ::DrawMeshInstanced(网格, *this, 变换矩阵数组, 实例个数);
  }

  /**
   * 检查材质是否准备就绪
   */
  [[nodiscard]] bool 检查可用性() const { return ::IsMaterialValid(*this); }
