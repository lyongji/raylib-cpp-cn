...
public:
   ...
   */
  窗口(int 宽, int 高, const std::string &标题 = "raylib-cn",
       unsigned int 初始化选项 = 0) {
    初始化(宽, 高, 标题, 初始化选项);
  }
   ...
   * @throws raylib::RaylibException 如果窗口初始化失败则抛出异常
   */
  static void 初始化(int 宽 = 800, int 高 = 450,
                     const std::string &标题 = "raylib-cn",
                     unsigned int 初始化选项 = 0) {
    if (初始化选项 != 0) {
      ::SetConfigFlags(初始化选项);
    }
   ...
   * 检查是否按下 ESC 键或点击关闭图标
   */
  static bool 检查是否已关闭() { return ::WindowShouldClose(); }
   ...
   * 检查光标是否在当前屏幕上
   */
  static bool 检查光标是否在屏上() { return ::IsCursorOnScreen(); }

  /**
   * 检查窗口是否当前处于全屏模式
   */
  static bool 检查是否正全屏() { return ::IsWindowFullscreen(); }

  /**
   * 检查窗口是否当前隐藏
   */
  static bool 检查是否已隐藏() { return ::IsWindowHidden(); }

  /**
   * 检查窗口是否当前最小化
   */
  static bool 检查是否已最小化() { return ::IsWindowMinimized(); }

  /**
   * 检查窗口是否当前最大化
   */
  static bool 检查是否已最大化() { return ::IsWindowMaximized(); }

  /**
   * 检查窗口是否当前聚焦
   */
  static bool 检查是否正聚焦() { return ::IsWindowFocused(); }

  /**
   * 检查窗口是否在上一个帧中被调整大小
   */
  static bool 检查是否被调整尺寸() { return ::IsWindowResized(); }

  /**
   * 检查是否启用了特定的窗口标志
   */
  static bool 检查是否有此配置项(unsigned int 选项) {
    return ::IsWindowState(选项);
  }

  /**
   * 使用标志设置窗口配置状态
   */
  窗口 &设配置项(unsigned int 选项) {
    ::SetWindowState(选项);
    return *this;
  }
   ...
   * 设置窗口在屏幕上的位置
   */
  窗口 &设位置(const ::Vector2 &位置) {
    return 设位置(static_cast<int>(位置.x), static_cast<int>(位置.y));
  }
  ...
  窗口 &设尺寸(const ::Vector2 &尺寸) {
    return 设尺寸(static_cast<int>(尺寸.x), static_cast<int>(尺寸.y));
  }
   ...
   */
  static Vector2 取尺寸() {
    return {static_cast<float>(取宽度()), static_cast<float>(取高度())};
  }
   ...
   * 设置剪贴板文本内容
   */
  static void 设剪贴板文本(const std::string &text) {
