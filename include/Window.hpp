...
public:
     ...
     */
    窗口(int 宽, int 高, const std::string& 标题 = "raylib-cn", unsigned int 初始化选项 = 0) {
        初始化(宽, 高, 标题, 初始化选项);
    }
     ...
     * @throws raylib::RaylibException 如果窗口初始化失败则抛出异常
     */
    static void 初始化(int 宽 = 800, int 高 = 450, const std::string& 标题 = "raylib-cn", unsigned int 初始化选项 = 0) {
        if (初始化选项 != 0) {
     ...
     * 检查光标是否隐藏
     */
    static bool 检查光标是否隐藏() { return ::IsCursorHidden(); }

    /**
     * 隐藏光标
     */
    static void 隐藏光标() { ::HideCursor(); }

    /**
     * 显示光标
     */
    static void 显示光标() { ::ShowCursor(); }
     ...
     * 检查是否启用了特定的窗口标志
     */
    static bool 检查是否有此配置项(unsigned int 选项) { return ::IsWindowState(选项); }
