...
public:
  ...
  包围盒(::Vector3 最大最小值 = ::Vector3{0.0f, 0.0f, 0.0f})
      : ::BoundingBox{最大最小值, 最大最小值} {}
   ...
   * 用线条绘制一个包围盒
   */
  void 绘制包围盒(::Color 颜色 = {255, 255, 255, 255}) const {
    ::DrawBoundingBox(*this, 颜色);
  }

  /**
   * 检测两个包围盒之间的碰撞
   */
  [[nodiscard]] bool 检测碰撞(const ::BoundingBox &包围盒2) const {
    return CheckCollisionBoxes(*this, 包围盒2);
  }

  /**
   * 检测包围盒与球体之间的碰撞
   */
  [[nodiscard]] bool 检测碰撞(::Vector3 中心, float 半径) const {
    return CheckCollisionBoxSphere(*this, 中心, 半径);
  }

  /**
   * 检测射线与包围盒之间的碰撞
   */
  [[nodiscard]] bool 检测碰撞(const ::Ray &射线) const {
    return GetRayCollisionBox(射线, *this).hit;
  }

  /**
   * 获取射线与包围盒之间的碰撞信息
   */
  RayCollision 取碰撞信息(const ::Ray &射线) const {
    return GetRayCollisionBox(射线, *this);
  }
