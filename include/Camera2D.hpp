...
public:
    ...
    GETTERSETTER(::Vector2, 目标位置, target)
    GETTERSETTER(float, 旋转角度, rotation)
     ...
     * 返回 2D 相机的变换矩阵
     */
    [[nodiscard]] Matrix 取变换矩阵() const { return ::GetCameraMatrix2D(*this); }

    /**
     * 返回 2D 相机屏幕空间位置对应的 2D 世界空间位置
     */
    [[nodiscard]] Vector2 转换屏幕到世界坐标(::Vector2 屏幕坐标) const { return ::GetScreenToWorld2D(屏幕坐标, *this); }

    /**
     * 返回 2D 世界空间位置对应的 2D 屏幕空间位置
     */
    [[nodiscard]] Vector2 转换世界到屏幕坐标(::Vector2 世界坐标) const { return ::GetWorldToScreen2D(世界坐标, *this); }
protected:
