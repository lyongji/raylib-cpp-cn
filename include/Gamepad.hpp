...
public:
     ...
     * 检测游戏手柄是否可用
     */
    [[nodiscard]] bool 是可用() const { return ::IsGamepadAvailable(编号); }
     ...
     * 检测游戏手柄按钮是否被按下一次
     */
    [[nodiscard]] bool 是按钮按下(int 按钮) const { return ::IsGamepadButtonPressed(编号, 按钮); }

    /**
     * 检测游戏手柄按钮是否正在被按下
     */
    [[nodiscard]] bool 是按钮按住(int 按钮) const { return ::IsGamepadButtonDown(编号, 按钮); }

    /**
     * 检测游戏手柄按钮是否被释放一次
     */
    [[nodiscard]] bool 是按钮释放(int 按钮) const { return ::IsGamepadButtonReleased(编号, 按钮); }

    /**
     * 检测游戏手柄按钮是否未被按下
     */
    [[nodiscard]] bool 是按钮未按下(int 按钮) const { return ::IsGamepadButtonUp(编号, 按钮); }
     ...
     * 返回游戏手柄轴的移动值
     */
    [[nodiscard]] float 取轴移动值(int 轴) const { return ::GetGamepadAxisMovement(编号, 轴); }

    static int 设映射(const std::string& 映射) { return SetGamepadMappings(映射.c_str()); }

    /**
     * 设置游戏手柄振动（两个电机的振动强度，持续时间以秒为单位）
     */
    void 设震动(float 左电机, float 右电机, float 持续时间) const {
