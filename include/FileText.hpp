...
public:
   ...
   * 从文件名加载文件文本
   */
  explicit 文件文本(const std::string &文件路径) { 加载(文件路径); }
   ...
   * 返回字符串形式的文本内容
   */
  [[nodiscard]] std::string ToString() const { return 数据; }
   ...
   * 从文件名加载文件文本
   */
  void 加载(const std::string &文件路径) { 加载(文件路径.c_str()); }

  /**
   * 从文件名加载文件文本
   */
  void 加载(const char *文件路径) {
    数据 = ::LoadFileText(文件路径);
    长度 = ::TextLength(数据);
  }
